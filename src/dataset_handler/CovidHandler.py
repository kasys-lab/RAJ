...
class CovidHandler(BaseHandler):
    def sample_eval_data(
        ...
        df_merged = df_merged.rename(
            columns={
                "text_x": "query_description",
                "text_y": "doc_abstract",
                "narrative": "query_narrative",
                "title": "doc_title",
            }
        )
        df_merged = df_merged.drop(columns=["iteration", "url", "pubmed_id"])
        df_merged = df_merged[df_merged["doc_abstract"] != ""].reset_index(drop=True)
